...
namespace NetCore.Common
{
    ...
    /// </summary>
    public readonly struct QuickLists
    {
        /// <summary>
    ...
    public struct QuickList<TBase> where TBase : class
    {
        static class BaseSet
        {
            ...
            {
                if (inUse >= QuickLists.ItemLimit)
                {
                    throw new Exception($"{nameof(QuickList<TBase>)} - exhausted all item IDs ({inUse}/{QuickLists.ItemLimit}).");
                }
        ...
        /// ===     ===     ===     ===    ===  == =  -                        -  = ==  ===    ===     ===     ===     ===]]>
        /// <summary>
        /// Retrieves amount of stored items in this <see cref="QuickList{TBase}"/>.
        /// </summary>
        ...
        {
            if (capacity < 0 || capacity >= QuickLists.ItemLimit)
                throw new ArgumentOutOfRangeException($"{nameof(QuickList<TBase>)} {nameof(capacity)} should be within a range: [0:{QuickLists.ItemLimit}]");
        ...
        public bool Insert<TItem>(int index, TItem item) where TItem : class, TBase
        {
            ...
            for (int i = 0; i < QuickLists.ItemLimit; i++)
            {
                if ((flags & (1u << i)) == 0)
                {
                    continue;
